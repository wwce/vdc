...
See link:https://docs.microsoft.com/azure/expressroute/expressroute-howto-circuit-portal-resource-manager[Create and modify an ExpressRoute circuit] for details on establishing an ExpressRoute.

|===

For testing purposes, the toolkit includes an archetype for a link:TODO[simulated on-premises environment] hosted on Azure. The toolkit will automatically connect this environment to your shared services network using a VPN gateway (Vnet-2-Vnet connection type).
...
See link:https://docs.microsoft.com/azure/architecture/reference-architectures/hybrid-networking/[Choose a solution for connecting an on-premises network to Azure] in the Azure Architecture Center for additional guidance.